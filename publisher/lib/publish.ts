import { composeMarkdown, relativeAsset } from './content';
import { ApiError, commitArticle, readArticle, type GitCall } from './github';
...
export async function publishDraft(
  ...
  if (names.length > 12) throw new ApiError(400, '每次发布最多支持 12 张新图片。');
  ...
  for (const name of names) {
    ...
    const assetPath = `docs/assets/publisher/${draft.id}/${name}`;
